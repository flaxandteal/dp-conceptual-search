...
def remove_nouns(text: str):
    """
    Removes nouns from a string
    :param text:
    :return:
    """
    tokens = text.split()
...
def encode_array(array: List[float]) -> str:
    ...
    :return:
    """
    base64_str = base64.b64encode(np.array(array).astype(dbig)).decode("utf-8")
    return base64_str


def cosine_sim(vec1, vec2) -> np.float:
    """
